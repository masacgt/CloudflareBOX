...
画面上の表示名はAPIのIDと異なります。Cloudflareダッシュボードでは、`account.read` は「Account Settings Read」、`workers-platform.write` は「Workers」の最後にある小文字の「edit」、`workers-r2.write` は「Workers R2 Storage」の「Edit」、`d1.write` は「D1」の「Edit」に対応します。Workersの「Bind」や「Workers Editor」は今回の構成では選択しません。
