...
7. OAuth で利用可能な Cloudflare アカウントが1件ならそのまま構築へ進み、複数なら Tray がアカウント名と ID の選択画面を表示します。
8. 選択した1アカウントへ workers.dev、R2、D1、Worker、schema、binding、cron を自動作成または修復します。
9. Windows の「Androidを追加」から pairing 情報を発行し、Android アプリへ取り込みます。2台目以降も同じ方法で追加します。
...
デスクトップアプリとして Authorization Code + PKCE（S256）を使用します。Client Secret は使用しません。既定 redirect は `http://127.0.0.1:53682/oauth/callback/` です。
...
Cloudflare 連携後は、その installation ID に属する資源だけを構築します。
...
Cloudflare OAuth の Public Client を登録し、Authorization Code + PKCE の loopback redirect を許可します。token endpoint authentication は `none`、PKCE は `S256` とします。OAuth Client に `account.read`、`workers-scripts.write`、`workers-r2.write`、`d1.write` を登録します。

公開 Client ID は GitHub repository variable `CLOUDFLAREBOX_OAUTH_CLIENT_ID` に設定します。Client Secret はデスクトップ配布物へ入れません。
...
CI ではソース、暗号処理、Range resume、複数アカウント選択コードのコンパイル、package assembly まで検証できます。実ユーザーの Cloudflare アカウントを使う OAuth 自動構築 E2E は別途必要です。
