...
internal sealed class CloudflareProvisioner : IDisposable
{
    ...
    private async Task<string> ResolveApiBaseAsync(StoredSettings settings, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(subdomain)) throw new InvalidOperationException("This Cloudflare account does not have a workers.dev subdomain. Create one once in Cloudflare and run repair again.");
    ...
    private async Task<JsonDocument> ReadResultAsync(HttpResponseMessage response, string message, CancellationToken ct)
    {
        ...
        if (!doc.RootElement.TryGetProperty("success", out var success) || !success.GetBoolean() || !doc.RootElement.TryGetProperty("result", out var result))
        {
            doc.Dispose();
