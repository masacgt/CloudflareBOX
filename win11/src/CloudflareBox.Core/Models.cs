...
public sealed record DeviceInfo(string Id, string Kind, string Name, long? RevokedAt, long? LastSeenAt, long CreatedAt);
public sealed record DeviceListResponse(IReadOnlyList<DeviceInfo> Devices);
public sealed record DiagnosticsResponse(bool Ok, string Version, bool D1, bool R2, IReadOnlyDictionary<string, int> ActiveDevices);
