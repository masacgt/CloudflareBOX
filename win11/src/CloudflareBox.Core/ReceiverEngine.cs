...
public sealed class ReceiverEngine(CloudflareBoxApiClient api, HttpClient downloadHttp, RSA encryptionPrivateKey, ClientSettings settings)
{
    ...
    public async Task<TransferResult> ReceiveOneAsync(PendingTransfer transfer, CancellationToken ct = default)
    {
        ...
        var signed = await api.GetDownloadUrlAsync(transfer.Id, ct);
        await ResumableDownloader.DownloadAsync(downloadHttp, new Uri(signed.Url), encryptedPath, transfer.EncryptedSizeBytes, ct);
