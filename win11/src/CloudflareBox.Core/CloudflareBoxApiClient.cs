...
public sealed class CloudflareBoxApiClient : IDisposable
{
    ...
    public CloudflareBoxApiClient(string apiBase)
        : this(apiBase, new HttpClientHandler())
    {
    }

    public CloudflareBoxApiClient(string apiBase, HttpMessageHandler handler)
    {
    ...
    public async Task DownloadEncryptedAsync(string transferId, string path, long expectedSize, CancellationToken ct = default)
    {
        ...
        if (File.Exists(path) && existing == expectedSize) return;
        ...
        if (append)
        {
            ValidateResumeResponse(response, existing, expectedSize);
        }
        else
        {
            existing = 0;
            ValidateFullResponse(response, expectedSize);
        }
    ...
    private static void ValidateResumeResponse(HttpResponseMessage response, long existing, long expectedSize)
    {
        ...
        if (contentRange is null ||
            ...
            contentRange.To != expectedSize - 1 ||
            contentRange.Length != expectedSize)
        {
            throw new InvalidDataException("Resume response Content-Range does not match the local partial file");
        }

        var remaining = expectedSize - existing;
        if (response.Content.Headers.ContentLength is long contentLength && contentLength != remaining)
        {
            throw new InvalidDataException($"Resume response length mismatch. expected={remaining} actual={contentLength}");
        }
    }

    private static void ValidateFullResponse(HttpResponseMessage response, long expectedSize)
    {
        if (response.StatusCode == HttpStatusCode.PartialContent)
        {
            ...
            if (contentRange is null ||
                ...
                contentRange.Length != expectedSize)
            {
                throw new InvalidDataException("Partial response does not describe the full requested object");
            }
        }

        if (response.Content.Headers.ContentLength is long contentLength && contentLength != expectedSize)
        {
            throw new InvalidDataException($"Download response length mismatch. expected={expectedSize} actual={contentLength}");
        }
    }
