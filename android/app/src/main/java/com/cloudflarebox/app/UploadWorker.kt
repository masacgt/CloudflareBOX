...
class UploadWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (item.state == "CANCELED" || item.state == "CANCEL_PENDING") return@withContext Result.success()
            ...
            if (state == "PAUSED" || state == "CANCELED" || state == "CANCEL_PENDING") return@withContext Result.success()
    ...
    private fun prepare(item: QueueItem, config: PairingConfig) {
        ...
        try {
            ...
            db.savePrepared(item.id, CryptoManager.protectLocal(fileKey), sha, wrapped, encryptedMetadata.nonceB64, encryptedMetadata.cipherB64)
        } finally {
            fileKey.fill(0)
        }
    }
    ...
    private suspend fun uploadParts(item: QueueItem, api: ApiClient) {
        ...
        try {
            ...
            while (nextPart <= item.partCount) {
                ...
                if (current.state == "CANCELED" || current.state == "CANCEL_PENDING") return
                ...
                try {
                    ...
                    api.uploadPart(remoteId, nextPart, partFile, encryptedHash)
                } finally {
                    partFile.delete()
                }
                nextPart++
    ...
    private fun openSource(source: String): InputStream = if (source.startsWith("content://")) {
        applicationContext.contentResolver.openInputStream(android.net.Uri.parse(source)) ?: throw IllegalStateException("source cannot be opened")
    } else {
