    ...
    private fun CloudflareBoxScreen() {
        ...
        Column(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    Button(onClick = {
                        scope.launch {
                            try {
                                ...
                                val status = ApiClient(config).status()
                                message = "PC状態を取得しました: ${status.optJSONObject("windows")?.optString("name", "Windows") ?: "未接続"}"
                            } catch (e: Exception) {
            ...
            LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                items(queue, key = { it.id }) { item -> TransferCard(item, scope = scope) }
            }
        }
    }

    @Composable
    private fun TransferCard(item: QueueItem, scope: kotlinx.coroutines.CoroutineScope) {
            ...
            Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    when (item.state) {
                        ...
                        "R2_READY", "CANCELED" -> Unit
                        else -> Button(onClick = {
                            db.setState(item.id, "PAUSED")
                        }) { Text("一時停止") }
                    }
                    if (item.state != "CANCELED" && item.state != "R2_READY") {
                        Button(onClick = {
                            WorkManager.getInstance(this@MainActivity).cancelUniqueWork(workName(item.id))
                            db.setState(item.id, "CANCELED")
                            if (item.remoteId != null) {
                                scope.launch {
                                    try {
                                        PairingStore.load(this@MainActivity)?.let { ApiClient(it).cancel(item.remoteId) }
                                    } catch (_: Exception) { }
                                }
                            }
                        }) { Text("キャンセル") }
